...
export class OptimizeRoutePlanUseCase {
  ...
  async execute(input: OptimizeRoutePlanInput): Promise<RoutePlan> {
    ...
    const vehicleIds = [...new Set(routePlan.routes.map(route => route.assignedVehicleId))];
    const stopIds = [...new Set(routePlan.routes.flatMap(route => route.orderedStops))];
    ...
    if (vehicles.length === 0 || stops.length === 0) {
        console.warn('No vehicles or stops associated with this plan to optimize.');
